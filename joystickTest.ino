...
It reports changing values for the configured joystick pins.  This includes
...
boolean deltaOverHysteresis ( unsigned int current, unsigned int proposed ) {
  ...
  // negative offsets.  The generated value could be negative, which would result
  // in a very large positive value with the unsigned variables.  The compare
  // would not work.  To avoid that, offset (add to) all values by the maximum
  // negative delta that could be used in the comparisons.  The comparison
  // expressions will then always be positive values.
